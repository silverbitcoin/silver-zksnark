...
impl ProofGenerator {
    ...
    pub fn load_proving_key(&self, key_data: Vec<u8>) -> Result<()> {
        ...
        let pk = Groth16ProvingKey::<Bn254>::deserialize_compressed(cursor).map_err(|e| {
            ZkSnarkError::SerializationError(format!("Failed to deserialize proving key: {}", e))
        })?;
    ...
    pub async fn generate_proof(
        ...
        if self.gpu_enabled {
            info!(
                "Proof generated with GPU acceleration in {}ms",
                generation_time_ms
            );
        } else {
        ...
        state_root[..current_state_root.len().min(64)]
            .copy_from_slice(&current_state_root[..current_state_root.len().min(64)]);

        let mut prev_proof_hash = [0u8; 64];
        prev_proof_hash[..previous_proof_hash.len().min(64)]
            .copy_from_slice(&previous_proof_hash[..previous_proof_hash.len().min(64)]);
    ...
    async fn generate_groth16_proof(&self, circuit: SnapshotCircuit) -> Result<Vec<u8>> {
        ...
        let proof = tokio::task::spawn_blocking(move || {
            ...
            Groth16::<Bn254>::prove(&pk_clone, circuit, &mut rng).map_err(|e| {
                ZkSnarkError::ProofGenerationFailed(format!(
        ...
        proof.serialize_compressed(&mut proof_bytes).map_err(|e| {
            ZkSnarkError::SerializationError(format!("Failed to serialize proof: {}", e))
        })?;
    ...
    pub fn generate_keys() -> Result<(Vec<u8>, Vec<u8>)> {
        ...
        // Generate keys
        let (pk, vk) =
            Groth16::<Bn254>::circuit_specific_setup(circuit, &mut rng).map_err(|e| {
                ZkSnarkError::ProofGenerationFailed(format!("Key generation failed: {}", e))
            })?;

        // Serialize proving key
        let mut pk_bytes = Vec::new();
        pk.serialize_compressed(&mut pk_bytes).map_err(|e| {
            ZkSnarkError::SerializationError(format!("Failed to serialize proving key: {}", e))
        })?;

        // Serialize verifying key
        let mut vk_bytes = Vec::new();
        vk.serialize_compressed(&mut vk_bytes).map_err(|e| {
            ZkSnarkError::SerializationError(format!("Failed to serialize verifying key: {}", e))
        })?;

        info!(
            "Keys generated: PK size = {} bytes, VK size = {} bytes",
            pk_bytes.len(),
            vk_bytes.len()
        );
